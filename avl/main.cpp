...
private:
  ...
  BinaryNode *root{};
  bool search2(int target, BinaryNode *node) {
    if (!node)
      return false;
    ...
    if (target < node->data) {
      return search2(target, node->left);
    }

    return search2(target, node->right);
  }
  ...
  bool is_bst(BinaryNode *node) {
    bool left_bst =
        !node->left || node->data > node->left->data && is_bst(node->left);

    if (!left_bst)
      return false;

    bool right_bst =
        !node->right || node->data < node->right->data && is_bst(node->right);
    return right_bst;
  }
  ...
  BinaryNode *lower_bound(int target, BinaryNode *node) {
    ...
    if (node->data < target) {
      return lower_bound(target, node->left);
    }

    return lower_bound(target, node->right);
  }
  // TODO: ask chatgpt if it is correct
  void lower_bound_void(int target, BinaryNode *node, BinaryNode *&ans) {
    if (!node)
      return;

    if (node->data < target) {
      lower_bound_void(target, node->left, ans);
    }

    if (node->data >= target) {
      ans = node;
      lower_bound_void(target, node->right, ans);
    }
  }
